...
list_line:
        if(lst){
            ...
            RIA.addr1 = (unsigned)(XRAM_OUT_BASE + (uint16_t)(line_pc_before - org));
            RIA.step1 = 1;
            for(a=line_pc_before; a<line_pc_after; ++a) fprintf(lst,"%02X ", (unsigned)RIA.rw1);
    ...
    if(len > 0){
        f = fopen("out.bin","wb"); if(!f){ printf("Writing error\n"); return; }
        {
            ...
            while(off < len){
                ...
                xram1_read_bytes((unsigned)(XRAM_OUT_BASE + (uint16_t)off), tmp, chunk);
                fwrite(tmp, 1, chunk, f);
                off += chunk;
            }
            fclose(f);
        }
        printf("\r\nSave in out.bin and out.lst\r\n");
    } else {
...
int main(int argc, char **argv){
    ...
    /*
    fd = open("hello.asm",O_RDONLY);
    if(fd){
        read_xram(0, 0x7FFF,fd);
        close(fd);
    }
    */
    pass1();
